...
SELECT genre, 
		COUNT(movie_id),
		RANK() OVER(ORDER BY 
			COUNT(movie_id) DESC) genre_rank
FROM genre
GROUP BY genre
)
SELECT * 
...
WHERE 
	m.title regexp "^The" and r.avg_rating > 8
GROUP BY  	
	g.genre, 
    r.avg_rating, 
    m.title
...
SELECT d.name_id,
		...
		total_votes,
		m.date_published,
		Lead(date_published,1) OVER(PARTITION BY 
        d.name_id 
        ORDER BY 
        date_published,movie_id ) AS next_date_published
